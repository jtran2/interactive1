...
function init() {
  ...
  if($.isFunction($.fn.fancybox)) {
    ...
    var t = $("#_ctoken:first");
    if (t.length > 0) {
      t.after( $("<input>", {"type": "hidden", "name": "_wu_token", "id": "_wu_token", "value": "R5uLS3VDIp"}) );
    }

    // Adding bot checkbox
    var hu = $(".captcha, .forum form .input, .GC_blogs .GC_honeypot");
    if(hu.length > 0) {
      hu.first().after('<li class="checkbox"><label><input type="checkbox" name="_w_bot"> I am not a bot</label> <span>Required</span></li>');
    }
  }
}
